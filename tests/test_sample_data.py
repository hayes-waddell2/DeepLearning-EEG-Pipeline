...
def test_both_splits_exist():
    for split in ["train", "eval"]:
        assert (SAMPLE_DATA_DIR / split).exists(), (
            f"Missing split directory: {SAMPLE_DATA_DIR / split}"
        )
...
def test_both_labels_per_split():
    for split in ["train", "eval"]:
        for label in ["normal", "abnormal"]:
            assert (SAMPLE_DATA_DIR / split / label).exists(), (
                f"Missing label directory: {SAMPLE_DATA_DIR / split / label}"
            )
...
def test_pipeline_saves_valid_npy(tmp_path):
    ...
    assert saved.dtype in [np.float32, np.float64], (
